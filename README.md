...
**Why the rendering seam is a trait and the video seam is not.** SDL routes every subsystem through a vtable (`SDL_VideoDevice`, `VideoBootStrap`) because it supports two dozen platforms. On a Windows-only target that indirection buys nothing and costs a dispatch on every window operation, so the video seam here is a *file* boundary: all Win32 lives under `video/windows/`, and porting means adding a sibling folder. Rendering is different — a second backend is genuinely reachable — so `RenderDevice` is a real trait with `SoftDevice` behind it.
